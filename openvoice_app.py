...
import langid
import se_extractor
from api import BaseSpeakerTTS, ToneColorConverter, AzureTTS
...
def predict(prompt, tts_engine, style, audio_file_pth):
    ...
    # time how long it takes to run TTS
    tts_start_time = perf_counter()
    tts_model.tts(prompt, src_path, speaker=style, language=language)
    tts_end_time = perf_counter()
    tts_elapse = tts_end_time - tts_start_time
    # print(f'TTS time: {tts_elapse}')

    save_path = f'{output_dir}/output.wav'
    # Run the tone color converter
    encode_message = "@MyShell"
    converter_start_time = perf_counter()
    ...
    converter_end_time = perf_counter()
    converter_elapse = converter_end_time - converter_start_time
    # print(f'Converter Time: {converter_elapse}')

    text_hint += f'''Get response successfully \n'''
    text_hint += f'TTS Elapse Time: {tts_elapse} \n'
    text_hint += f'Converter Elapse Time: {converter_elapse} \n'
...
with gr.Blocks(analytics_enabled=False) as demo:
    ...
    with gr.Row():
        with gr.Column():
            ...
            tts_gr = gr.Dropdown(
                label="TTS",
                info="Select a TTS engine for the synthesised speech.",
                choices=['VITS', 'Azure'],
                max_choices=1,
                value="VITS",
            )
        ...
        with gr.Column():
            ...
            tts_audio_gr = gr.Audio(label="Un-converted Audio")

            gr.Examples(examples,
                        label="Examples",
                        inputs=[input_text_gr, style_gr, ref_gr],
                        outputs=[out_text_gr, tts_gr, audio_gr, ref_audio_gr, tts_audio_gr],
                        fn=predict,
                        cache_examples=False,)
            tts_button.click(predict, [input_text_gr, tts_gr, style_gr, ref_gr], outputs=[out_text_gr, audio_gr, ref_audio_gr, tts_audio_gr])

demo.queue()  
demo.launch(debug=True, show_api=True, share=args.share, server_name='0.0.0.0', server_port=7865)
